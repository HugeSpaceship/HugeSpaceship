...
func (w DeferredWriter) Write(data []byte) (int, error) {
	digestKey := w.cfg.GameAPI.DigestKey
	if w.alternateDigest {
		digestKey = w.cfg.GameAPI.AlternateDigestKey
	}
...
func GetRequestDigest(cfg *config.Config, path, digestHeader, cookie string, body []byte, excludeBody bool) (string, bool, error) {
	d := digest.CalculateDigest(path, cookie, cfg.GameAPI.DigestKey, body, excludeBody)

	alternateDigest := false

	if d != digestHeader && digestHeader != "" {
		d = digest.CalculateDigest(path, cookie, cfg.GameAPI.AlternateDigestKey, body, excludeBody)
		alternateDigest = true
		if d != digestHeader {
			if cfg.GameAPI.EnforceDigest {
...
func DigestMiddleware(cfg *config.Config) func(next http.Handler) http.Handler {
	return func(next http.Handler) http.Handler {
		fn := func(w http.ResponseWriter, r *http.Request) {
			...
			if cfg.GameAPI.EnforceDigest && err != nil { // Digest failed to authenticated
				slog.Info("Aborting request due to invalid digest", "client", r.RemoteAddr)
				utils.HttpLog(w, http.StatusForbidden, "Failed to authenticate digest.")
			} else if err != nil {
				slog.Debug("Invalid digest, continuing anyway", "client", r.RemoteAddr)
			}
