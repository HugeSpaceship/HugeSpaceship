...
// TODO: rewrite this into a proper resource handling package
func getResourceFromCache(cfg *config.Config, conn *pgxpool.Conn, hash string) (string, bool) {
...
func GetResourceHandler(cfg *config.Config) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if cfg.ResourceServer.CacheResources { // check for cache
			if resourceFile, exists := getResourceFromCache(cfg, conn, hash); exists {
		...
		resource, tx, size, err := hs_db.GetResource(conn, hash)
		if err != nil {
